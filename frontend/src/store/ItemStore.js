...
const useItemStore = create((set) => ({
  ...
  fetchItems: async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_APP_ITEM_SERVICE_URL}/items/getitems`, {
  ...
  createItem: async (userId, formData) => {
    try {
      ...
      await axios.post(`${import.meta.env.VITE_APP_ITEM_SERVICE_URL}/items/create`, requestData, {
        withCredentials: true,
      });
